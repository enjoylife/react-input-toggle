...
class FancySwitch extends React.Component {
  ...
  static propTypes = {
    ...
    label: PT.string,
    labelPostion: PT.oneOf(['top', 'right', 'bottom', 'left']),

    effectName: PT.oneOf(Object.keys(switchStyles)),
  ...
  render () {
    ...
    const outerCss = cs('fi-switch', 'fi-switch--' + effectName, 'fi-switch--label-' + labelPostion, isFocused == true && 'fi-switch--focused', checked == true && 'fi-switch--checked');

    const labelCss = cs([
      'fi-switch__label',
      'fi-switch__label--' + effectName
    ]);
